...
detected=None
fps = 60
isClassifying=True # if automatic contour detection to neural network detection is on
key=cv.waitKey(1)
...
while(not (key & 0xFF == ord('q'))): # main loop
    ...
    if classifying:
        ...
        # DNN detection
        elif isClassifying and (len(gameObjects)>0) and abs(int(time.time())-lastTime)>10:
            lastTime = int(time.time())
